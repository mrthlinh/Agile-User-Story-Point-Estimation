...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 20 13:23:38 2018
...
#    embedding_reshape = tf.reshape(embedding, [1,1,tf.shape(labels),sentence_size,embedding_size]) 
    ...
    # Average Pooling
    embedding_avg = tf.layers.average_pooling3d(inputs = embedding_reshape, pool_size = [1,sentence_size,1],strides=1,data_format='channels_first')
     
    # Add one layer of fully connected layer for regression
    FC = tf.layers.dense(inputs=embedding_avg, use_bias = True, units=1, name = "predicted_points")
    
    outputs = tf.reshape(FC, [-1,1])
    
#    outputs = tf.layers.flatten(FC)
...
def main(unused_argv):
    ...
    # Create the Estimator -> Re-use last-modeol
...
#    train_results = RNN_classifier.evaluate(
#       input_fn = lambda: eval_input_fn(x_train,y_train,batch_size=args.batch_size))
